...
class Diffuser:
  ...
  def predict_x_zero(self, x_t, label, noise_level):
    """
    Predict original image based on noisy image (or matrix of noisy images) 
    at noise level plus conditional label
    """
  ...
  def reverse_diffusion(self, seeds, label, show_img=False):
    """
    Reverse Guided Diffusion on a matrix of random images (seeds). 
  ...
  def reverse_diffusion_masked(self, seeds, label_ohe, show_img=False, masked_imgs=None, mask=None, u=1):
    """
    Reverse Guided Diffusion on a matrix of random images (seeds) with a mask. 
    Can be used for in/outpainting
      ...
      for j in range(1, u + 1):
        ...
        if masked_imgs is not None:
          ...
          if i <= int(self.diffusion_steps * 0.8):
            ####masked part:
            new_img_known = (1 - next_noise) * masked_imgs + np.random.normal(0, 1, size=new_img.shape) * next_noise
        ...
        if j != u:
          ...
          new_img = s * new_img + np.sqrt(curr_noise ** 2 - s ** 2 * next_noise ** 2) * np.random.normal(0, 1, size=new_img.shape)

      if show_img:
        plot_images(x0_pred, 
                    nrows=np.sqrt(len(new_img)),
                    save_name=str(i),
                    size=12)
        plt.show()
